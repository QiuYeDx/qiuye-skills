...
3. 判断改动类型、规模、风险和探索需求。涉及页面、组件、样式或交互时，在写相关前端代码前必读 [frontend-quality](references/frontend-quality.md)，选择可用设计 Skill 或内置流程。已有 Spec 时先定位当前批次、批准依据、任务与真实代码基线。
...
9. **前端先设计，渲染后审查。** 开工前明确参照、信息层级、布局、组件与状态；初版后实际查看渲染结果、执行交互，修复可发现问题并复验。元素存在、截图生成或构建通过不能单独证明 UI/UX 合格。

## 流程 S：不要把小任务变成文档项目

读相关代码 → 写简短目标/验收/影响面 → 按风险确认 → 实现 → 运行针对性验证 → 报告结果与缺项。
UI 小改在实现前补简短参照与设计决定，初版后审查受影响的渲染状态并修复复验；可只记在对话中，不扩成整页设计或全站回归。
...
2. 写 design.md 的当前方案、实际代码位置、取舍与验证风险；UI 部分先完成 [前端设计基准](references/frontend-quality.md)，不能只有技术选型或“沿用项目风格”。
3. 在 tasks.md 拆可验证的增量；先做最小端到端链路，不以“一个聊天会话”为工作边界。UI 任务纳入设计依据、真实渲染审查和修复复验，不将打磨留作无负责人后续项。
...
当前批次的设计与计划通过 ready 和语义审查后到 CP2；涉及 UI 时同样先确定当前页面/组件的设计基准，不因拆成前端任务而跳过。不要先写完整个项目再验证第一条链路。
...
1. 读当前 AC、设计、任务及必要契约；检查批准/授权、未提交改动、依赖在本工作基线是否实际可用。UI 任务先核对设计基准和验证样本；缺失时补齐再写相关前端代码，沿用已有授权处理可逆设计细节。
2. 按任务真实写集领取一个任务，标进行中；共享文件由唯一负责人处理。任务 ID 不是锁，Git 分支也不是锁。
3. 实现并同步必要设计。范围内的文件落点调整可说明后更新写集；跨负责人路径先协调；外部行为变化进入变更管理。
4. 执行任务验证计划：逐个记录命令/步骤、版本、环境、结果、证据。UI 初版后切换到审查视角，查看截图和实际交互，按方向→结构→细节→交互→适配检查，修复后重看受影响状态；证据对应最终代码。检查与未执行项都不得省略。
...
| ready | 当前 AC/设计/任务/验证计划完整、没有当前阻断问题；UI 语义审查包含设计基准及渲染验收计划。通过不代表已获准实施 |
| 实施前 | 当前范围有真实批准或明确授权；high 风险需具体批准，笼统“不用问”不是不可逆操作授权 |
| 任务 done | required 检查和 AC 有逐条证据、日期和实际验证版本；UI 已完成渲染审查与必要修复复验，过期证据要重新评估 |
| 批次验收 | 在集成版本上完成适用的用户场景与跨模块验证，记录真实确认人；不是各任务状态之和 |

详见 [quality-gates](references/quality-gates.md)。UI 任务必读 [frontend-quality](references/frontend-quality.md)；接口/数据任务读 [backend-quality](references/backend-quality.md)。
