...
写相关前端代码前，先明确用户任务、具体页面/token/组件参照、布局与空间分配、状态和验收样本。优先读取适用的已安装设计 Skill，如 `qiuye-ui-quality`；缺失时可参考 [仓库中的 Skill](https://github.com/qiuyedx/qiuye-skills/tree/main/frontend/ui-ux/skills/qiuye-ui-quality)，离线时使用内置 [前端质量流程](references/frontend-quality.md)。

初版后实际查看渲染截图并操作用户流程，按方向、结构、细节、交互与适配审查；修复发现的问题，再复验受影响状态，直到最终证据对应最终代码。没有问题无需强制二次改代码；没有浏览器不能冒充视觉通过。

S 可用几行记录；M/L 纳入已有 design/tasks/record，模板提示同步落实。v2.1 不新增强制设计文档或审批轮次，`schema_version` 仍为 2，CLI 参数、状态和指纹算法不变。当前 UI 任务按新要求补齐缺项，旧验收记录保留历史事实。检查器不自动判定 UI 任务或审美质量，设计门与渲染门仍须真实审查。
