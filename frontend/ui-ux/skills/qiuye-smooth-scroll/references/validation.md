...
模拟 iPhone UA 与 iPadOS 的 MacIntel + 多触摸点组合，确认根控制器没有创建 Lenis、注册自身触摸监听或包装原生滚动 API；仅检查没有 `.lenis` class 不够。重挂载、路由变化及 reduced motion 切换后也应保持跳过。回归普通 Mac、Windows 触摸设备和 Android，避免平台识别扩大禁用范围；使用 `disableOnIOS: false` 的项目还需验证覆盖选项实际生效。

真机上比较相同页面和动效配置下的慢拖、快速甩动、反向及松手惯性，重点观察滚动联动区域。记录设备、系统、浏览器版本与用户感受；模拟 UA 只能证明分支选择，不能证明 iOS 手感。反馈只有小幅改善时如实记录，继续区分输入等待与主线程/GPU 开销，不能写成“已根治”。
