...
| 空间关系 | 经过不同容器层级但应对齐的元素 | 相对共同参照的边缘、列与基线；表面、热区和文本分别判断 |
...
| 状态组合 | 相邻对象的不同状态；同一对象同时成立的状态 | 强调的主次、绘制范围、边界及独立语义是否仍可辨认 |
| 状态转换 | 会增减内容、操作或提示区域的关键转换前后 | 工作区、下一操作目标、滚动锚点和焦点是否受到不必要扰动 |
| 共享组件 | 消费页与实际使用的各个 variant | 局部覆盖、嵌套边距、窄窗口和状态组合 |

不必测试所有状态的笛卡尔积。优先选择可能改变布局或操作含义的组合：最长文案 + 最窄受支持窗口、同一对象的多种强调、正文多行 + 操作按钮、Popover + 靠近窗口边缘。样本规模应足以检验本任务的操作成本和作用范围，不等于每次都做性能压测或补齐集合管理功能。
...
比较相关几何关系时，在同一稳定帧读取共同参照和目标矩形，避免先后测量跨越布局或缩放变化。检查状态组合时，不只读外层背景，还要查看内部元素、伪元素与主题覆盖的实际绘制范围，结合截图判断重复强调；键盘焦点等独立反馈不能被误删。

对会改变结构的转换，保持窗口和数据条件可比，记录前后的工作区尺寸、目标位置、滚动与焦点。稳定位置是设计目标时才断言不变；有意重排应验证内容仍可用、上下文可追踪、下一步仍易操作。不要将固定栏、固定高度或零位移作为所有界面的验收标准。
...
- 组合与转换：让多个相关状态同时成立，并对比关键转换前后；各状态单独通过不能证明组合或过程合格。
